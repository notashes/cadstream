use cadstream::{cad_data::Triangle, stl_parser::StlParser};
...
fn bench_ascii_parsing(c: &mut Criterion) {
    ...
    for &count in &triangle_counts {
        ...
        group.bench_with_input(BenchmarkId::new("triangles", count), &data, |b, data| {
            let parser = StlParser::new();
            b.iter(|| {
                parser
                    .parse_data(data, format!("test_{}.stl", count))
                    .unwrap()
            });
        });
    }
