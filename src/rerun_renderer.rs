...
impl RerunRenderer {
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        // Log initial state immediately
        {
            ...
            if let Some(model) = model.as_ref() {
                ...
                println!("📊 Initial data logged to Rerun: {} triangles", last_triangle_count);
            } else {
    ...
    async fn log_model(&self, model: &CadModel) -> Result<()> {
        ...
        let vertices: Vec<[f32; 3]> = model.triangles
            .iter()
        ...
        self.rec.log(
            ...
            &rr::Boxes3D::from_centers_and_sizes([
                [(bbox_min.x + bbox_max.x) / 2.0, (bbox_min.y + bbox_max.y) / 2.0, (bbox_min.z + bbox_max.z) / 2.0]
            ], [
                [bbox_max.x - bbox_min.x, bbox_max.y - bbox_min.y, bbox_max.z - bbox_min.z]
            ])
            .with_colors([[204u8, 204u8, 204u8, 76u8]])
        )?;

        println!("📊 Updated Rerun: {} triangles from {}", model.triangles.len(), model.name);
